...
from pal_found_cli.common.error_serializer import EXIT_AUTH, EXIT_CONFIGURATION
...
def _ns(**kwargs):
    ...
    for spec in pal_found_admin_cli.OP_SPECS:
...
def _value_for(name):
    if name in pal_found_admin_cli.JSON_ARGS:
        if name in {"attributes", "where", "initial_permissions"}:
            return '{"k": "v"}'
        return '["x"]'
    if name in pal_found_admin_cli.BOOLEAN_ARGS:
        return True
    if name == "page_size":
        return 25
    if name == "page_token":
...
def test_catalog_matches_design_009_counts():
    ...
    actual = {
        ...
        for resource, operations in pal_found_admin_cli.OPERATION_BY_RESOURCE.items()
    }
    assert actual == expected


@pytest.mark.parametrize("spec", pal_found_admin_cli.OP_SPECS)
def test_parser_accepts_every_canonical_operation(spec):
    parser = pal_found_admin_cli.build_parser()
...
async def test_invoke_dispatches_every_canonical_operation(spec):
    ...
    await pal_found_admin_cli._invoke(
        spec["resource"],
        spec["operation"],
        client,
        args,
...
async def test_invoke_parses_json_and_boolean_args():
    ...
    await pal_found_admin_cli._invoke("group_member", "list", client, args, timeout=7)
...
async def test_invoke_decodes_all_design_json_args():
    for arg_name in pal_found_admin_cli.JSON_ARGS:
        spec = next(spec for spec in pal_found_admin_cli.OP_SPECS if arg_name in spec["json_args"])
        client = MagicMock()
        method = AsyncMock(return_value={"ok": True})
        setattr(client, spec["method"], method)
        args = _args_for(spec)
...
def test_get_client_routes_admin_root_and_nested_clients():
    ...
    assert pal_found_admin_cli._get_client(cfg, "user", factory) == sdk.admin.User
    assert (
        pal_found_admin_cli._get_client(cfg, "group_member", factory)
        == sdk.admin.Group.GroupMember
    )
    assert (
        pal_found_admin_cli._get_client(cfg, "authentication_provider", factory)
        == sdk.admin.Enrollment.AuthenticationProvider
    )


def test_paginated_catalog_matches_design_009():
    assert pal_found_admin_cli.PAGINATED_OPS == {
        ("group", "list"),
        ("group", "search"),
        ("group_member", "list"),
...
@pytest.mark.parametrize("resource,operation", sorted(pal_found_admin_cli.PAGINATED_OPS))
@pytest.mark.asyncio
async def test_invoke_paginated_uses_helper_for_all_paginated_ops(resource, operation):
    client = MagicMock()
    method_name = pal_found_admin_cli._spec_for(resource, operation)["method"]
    method = MagicMock(
    ...
    args = _args_for(pal_found_admin_cli._spec_for(resource, operation), page_size=1)
    helper = pal_found_admin_cli.PaginationHelper(page_size=1, batch_pages=2)

    result = await pal_found_admin_cli._invoke_paginated(
...
async def test_main_paginated_operation_emits_next_page_token(monkeypatch, capsys):
    ...
    monkeypatch.setattr(pal_found_admin_cli, "ConfigLoader", _Cfg)
    monkeypatch.setattr(pal_found_admin_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_admin_cli, "AccessControlGuard", lambda cfg, ns: MagicMock()
    )
    monkeypatch.setattr(pal_found_admin_cli, "AsyncClientFactory", lambda: factory)
    monkeypatch.setattr(
        sys,
        "argv",
...
async def test_main_success_uses_admin_acl_retry_output_and_b3_scope(monkeypatch, capsys):
    ...
    monkeypatch.setattr(pal_found_admin_cli, "ConfigLoader", _Cfg)
    monkeypatch.setattr(pal_found_admin_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(pal_found_admin_cli, "AccessControlGuard", lambda cfg, ns: guard)
    monkeypatch.setattr(pal_found_admin_cli, "AsyncClientFactory", lambda: factory)
...
async def test_main_acl_denied_returns_exit_8_and_skips_sdk(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda cfg, ns: MagicMock(
            check=MagicMock(
                side_effect=pal_found_admin_cli.AccessControlError("blocked")
            )
        ),
    )
    ...
    assert rc == pal_found_admin_cli.EXIT_ACCESS_CONTROL
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (FileNotFoundError("missing"), pal_found_admin_cli.EXIT_NOT_FOUND),
        (
            pal_found_admin_cli.AccessControlError("blocked"),
            pal_found_admin_cli.EXIT_ACCESS_CONTROL,
        ),
