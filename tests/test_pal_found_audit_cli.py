...
from pal_found_cli.common.binary_download_handler import InvalidDownloadError
...
from pal_found_cli.common.tracing_provider import TracingProvider
...
def _patch_main_dependencies(
    ...
    *,
    ...
    monkeypatch.setattr(pal_found_audit_cli, "ConfigLoader", cfg_type)
    monkeypatch.setattr(pal_found_audit_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_audit_cli,
        "AccessControlGuard",
        lambda cfg, namespace, **kwargs: selected_guard,
    )
    monkeypatch.setattr(pal_found_audit_cli, "AsyncClientFactory", lambda: factory)
    monkeypatch.setattr(
        pal_found_audit_cli,
        "RetryHandler",
...
def test_catalog_contains_exact_two_unique_nested_operations() -> None:
    assert len(pal_found_audit_cli.OP_SPECS) == 2
    assert {
        (
            spec["resource"],
            spec["operation"],
        ...
        for spec in pal_found_audit_cli.OP_SPECS
    } == {
        ("log_file", "list", "Organization.LogFile", "list"),
        ("log_file", "content", "Organization.LogFile", "content"),
    }
    assert set(pal_found_audit_cli.OPERATION_BY_RESOURCE) == {"log_file"}
...
def test_parser_accepts_every_declared_argument(
    ...
    args = pal_found_audit_cli.build_parser().parse_args(argv)
...
def test_help_exits_zero_and_names_operations(
    ...
    with pytest.raises(SystemExit) as exc:
        pal_found_audit_cli.build_parser().parse_args(argv)
...
async def test_missing_command_returns_one_without_loading_config(
    ...
    monkeypatch.setattr(pal_found_audit_cli, "ConfigLoader", loader)
    monkeypatch.setattr(sys, "argv", argv)
    assert await pal_found_audit_cli.main() == EXIT_USER_INPUT
...
def test_parse_iso_date_accepts_none_and_real_dates(
    ...
    assert pal_found_audit_cli._parse_iso_date(value, field="start_date") == expected
...
def test_parse_iso_date_rejects_non_strict_or_impossible_dates(value: str) -> None:
    with pytest.raises(ValueError, match="start_date"):
        pal_found_audit_cli._parse_iso_date(value, field="start_date")
...
def test_initial_list_requires_start_date(token: str | None) -> None:
    with pytest.raises(ValueError, match="start_date is required"):
        pal_found_audit_cli._validate_list_cursor(None, token)


def test_continuation_token_allows_missing_start_date() -> None:
    pal_found_audit_cli._validate_list_cursor(None, "cursor")


@pytest.mark.parametrize("value", [1, 30, 3600])
def test_timeout_accepts_adr_002_bounds(value: int) -> None:
    assert pal_found_audit_cli._validate_timeout(value) == value
...
def test_timeout_rejects_values_outside_adr_002_bounds(value: Any) -> None:
    with pytest.raises(ValueError, match="between 1 and 3600"):
        pal_found_audit_cli._validate_timeout(value)
...
async def test_fetch_list_page_uses_raw_wrapper_and_decodes_sdk_page() -> None:
    ...
    result = await pal_found_audit_cli._fetch_list_page(
        client,
...
async def test_list_default_fetches_one_raw_server_page_and_keeps_cursor() -> None:
    ...
    records, helper = await pal_found_audit_cli._list_log_files(
...
async def test_pagination_retry_restarts_helper_without_duplicate_counts() -> None:
    ...
    records, helper = await handler.execute(
        pal_found_audit_cli._list_log_files,
        _LogFileClient(raw=Endpoint()),
...
def test_packaged_metadata_policy_is_cwd_independent(
    ...
    monkeypatch.setenv("FOUNDRY_AGENTIC_CLI_METADATA_ONLY", "true")
    assert pal_found_audit_cli._METADATA_ALLOWLIST_PATH.is_file()
    guard = AccessControlGuard(
        _Cfg(),
        "AUDIT",
        metadata_allowlist_path=str(pal_found_audit_cli._METADATA_ALLOWLIST_PATH),
    )
    guard.check("log_file", "list")
...
async def test_acl_runs_before_factory_construction(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(pal_found_audit_cli, "ConfigLoader", _Cfg)
    monkeypatch.setattr(pal_found_audit_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_audit_cli,
        "AccessControlGuard",
    ...
    monkeypatch.setattr(pal_found_audit_cli, "AsyncClientFactory", factory_constructor)
    monkeypatch.setattr(
        sys,
        "argv",
        ["prog", "log-file", "content", "organization", "file"],
    )
    assert await pal_found_audit_cli.main() == EXIT_ACCESS_CONTROL
...
async def test_content_streams_unknown_length_with_one_byte_probe(
    ...
    result = await pal_found_audit_cli._download_content(
...
async def test_content_passes_all_unavailable_headers_as_none(
    ...
    monkeypatch.setattr(pal_found_audit_cli, "BinaryDownloadHandler", Handler)
    response = _PublicStreamResponse(_ByteStream([b"data"]))
    await pal_found_audit_cli._download_content(
...
async def test_content_retry_removes_failed_partial_before_publishing_success(
    ...
    result = await handler.execute(
        pal_found_audit_cli._download_content,
        _LogFileClient(streaming=Endpoint()),
        _content_args(),
        5,
        _DownloadCfg(tmp_path, 20),
...
async def test_main_list_outputs_data_then_pagination_metadata_on_stderr(
    ...
    assert await pal_found_audit_cli.main() == 0
    captured = capsys.readouterr()
...
async def test_main_passes_selected_timeout_to_retry_and_sdk(
    ...
    monkeypatch.setattr(pal_found_audit_cli, "RetryHandler", retry_factory)
    argv = [
        "prog",
        "log-file",
        "list",
...
async def test_main_rejects_timeout_before_acl_or_client(
    ...
    monkeypatch.setattr(pal_found_audit_cli, "ConfigLoader", _Cfg)
    monkeypatch.setattr(pal_found_audit_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(pal_found_audit_cli, "AccessControlGuard", guard_constructor)
    monkeypatch.setattr(pal_found_audit_cli, "AsyncClientFactory", factory_constructor)
    monkeypatch.setattr(
        sys,
        "argv",
...
async def test_main_serializes_exact_adr_exit_codes(
    ...
    assert await pal_found_audit_cli.main() == expected
...
async def test_main_maps_async_cancellation_to_timeout(
    ...
    assert await pal_found_audit_cli.main() == EXIT_TIMEOUT
...
async def test_b3_transport_headers_enabled_disabled_retry_stable_and_restored(
    ...
    monkeypatch.setattr(pal_found_audit_cli, "ConfigLoader", lambda: cfg)
    monkeypatch.setattr(pal_found_audit_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_audit_cli,
        "AccessControlGuard",
    ...
    monkeypatch.setattr(pal_found_audit_cli, "AsyncClientFactory", lambda: factory)
    monkeypatch.setattr(pal_found_audit_cli, "RetryHandler", lambda **kwargs: retry)
    monkeypatch.setattr(
        sys,
        "argv",
...
async def test_b3_scope_restores_prior_values_after_formatter_failure(
    ...
    monkeypatch.setattr(pal_found_audit_cli, "OutputFormatter", lambda **kwargs: formatter)
    retry = MagicMock()
    ...
    try:
        assert await pal_found_audit_cli.main() == EXIT_SERVER_ERROR
...
def test_source_never_uses_eager_content_private_sdk_fields_or_w3c() -> None:
    source = Path(pal_found_audit_cli.__file__).read_text(encoding="utf-8")
