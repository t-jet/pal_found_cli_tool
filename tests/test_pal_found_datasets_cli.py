#!/usr/bin/env python3
"""Unit tests for pal_found_datasets_cli.py — 33 operations across 5 resource clients.
...
_SKILL_DIR = (
    Path(__file__).parent.parent / ".agents" / "skills" / "pal-found-datasets" / "scripts"
)
sys.path.insert(0, str(_SKILL_DIR))
import importlib

pal_found_datasets_cli = importlib.import_module("pal_found_datasets_cli")
_model_to_dict = pal_found_datasets_cli._model_to_dict
_get_client = pal_found_datasets_cli._get_client
_invoke = pal_found_datasets_cli._invoke
_resolve = pal_found_datasets_cli._resolve
build_parser = pal_found_datasets_cli.build_parser
...
class TestGetClient:
    ...
    def test_dataset_returns_dataset_client(self):
        ...
        with patch.object(
            pal_found_datasets_cli.AsyncClientFactory, "create", return_value=mock_client
        ):
...
class TestMainIntegration:
    ...
    async def test_main_returns_user_input_when_no_resource(self, capsys):
        ...
        with patch.object(sys, "argv", ["prog"]):
            rc = await pal_found_datasets_cli.main()
    ...
    async def test_main_resolves_timeout_from_cli_flag(self):
        ...
        with (
            patch.object(pal_found_datasets_cli, "_get_client", return_value=mock_client),
            patch.object(pal_found_datasets_cli, "ConfigLoader") as cfg_cls,
            patch.object(pal_found_datasets_cli, "AccessControlGuard") as guard_cls,
            patch.object(pal_found_datasets_cli, "RetryHandler") as rh_cls,
            patch.object(pal_found_datasets_cli, "LogSetup"),
            patch.object(pal_found_datasets_cli, "OutputFormatter") as of_cls,
        ...
        assert args_passed[0] is pal_found_datasets_cli._invoke
    ...
    async def test_main_access_control_denied(self):
        ...
        with (
            ...
            patch.object(pal_found_datasets_cli, "AccessControlGuard") as guard_cls,
            patch.object(pal_found_datasets_cli, "LogSetup"),
            patch.object(pal_found_datasets_cli, "ErrorSerializer") as ser_cls,
        ):
            cfg = MagicMock()
            cfg.timeout_s = 30
            cfg.log_level = "INFO"
    ...
    async def test_main_handles_unknown_operation_as_server_error(self):
        ...
        with (
            ...
            patch.object(pal_found_datasets_cli, "LogSetup"),
            patch.object(pal_found_datasets_cli, "ErrorSerializer") as ser_cls,
            patch.object(pal_found_datasets_cli, "OutputFormatter"),
        ):
            cfg = MagicMock()
            cfg.timeout_s = 30
            cfg.log_level = "INFO"
            ...
            with patch.object(sys, "argv", ["prog", "dataset", "get", "rid"]):
                ...
                with patch.object(
                    pal_found_datasets_cli, "_resolve", return_value="bogus"
                ):
                    rc = await pal_found_datasets_cli.main()
...
def _patch_main_infra(monkeypatch, *, cfg_timeout=30):
    ...
    monkeypatch.setattr(
        pal_found_datasets_cli, "ConfigLoader", lambda: _StubCfg(cfg_timeout)
    )
    monkeypatch.setattr(pal_found_datasets_cli, "LogSetup", _StubLog)
    monkeypatch.setattr(pal_found_datasets_cli, "AccessControlGuard", _StubGuard)
    return monkeypatch
...
async def test_main_permission_error_path(monkeypatch, capsys):
    ...
    mc = _async_client()
    monkeypatch.setattr(pal_found_datasets_cli, "_get_client", lambda c, r: mc)
    rh = MagicMock()
    rh.execute = AsyncMock(side_effect=PermissionError("403"))
    monkeypatch.setattr(pal_found_datasets_cli, "RetryHandler", lambda: rh)
...
class TestPathResolution:
    ...
    def test_project_root_is_string(self):
        assert isinstance(pal_found_datasets_cli._PROJECT_ROOT, str)
        assert len(pal_found_datasets_cli._PROJECT_ROOT) > 0
