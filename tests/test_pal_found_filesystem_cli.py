...
def _ns(**kwargs):
    ...
    for spec in pal_found_filesystem_cli.OP_SPECS:
...
@pytest.mark.parametrize("spec", pal_found_filesystem_cli.OP_SPECS)
def test_parser_accepts_every_canonical_operation(spec):
    parser = pal_found_filesystem_cli.build_parser()
...
async def test_invoke_dispatches_every_canonical_operation(spec):
    ...
    await pal_found_filesystem_cli._invoke(
        spec["resource"],
        spec["operation"],
        client,
        args,
...
def test_get_client_routes_resource_role_nested_client():
    ...
    assert (
        pal_found_filesystem_cli._get_client(cfg, "resource_role", factory)
...
def test_paginated_catalog_matches_page_size_and_token_specs():
    ...
    assert pal_found_filesystem_cli.PAGINATED_OPS == expected
    assert expected == {
        ("folder", "children"),
...
async def test_invoke_paginated_uses_helper_for_all_paginated_ops(
    ...
    setattr(client, pal_found_filesystem_cli._spec_for(resource, operation)["method"], method)
    helper = pal_found_filesystem_cli.PaginationHelper(page_size=1, batch_pages=2)

    result = await pal_found_filesystem_cli._invoke_paginated(
...
async def test_main_paginated_operation_emits_next_page_token(monkeypatch, capsys):
    ...
    monkeypatch.setattr(pal_found_filesystem_cli, "ConfigLoader", _Cfg)
    ...
    monkeypatch.setattr(pal_found_filesystem_cli, "AsyncClientFactory", lambda: factory)
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_output_formatter_supports_json_toon_and_auto(format_setting, expected):
    ...
    output = pal_found_filesystem_cli.OutputFormatter(
...
async def test_main_success_uses_acl_retry_output_and_b3_scope(monkeypatch, capsys):
    ...
    monkeypatch.setattr(pal_found_filesystem_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_filesystem_cli, "AccessControlGuard", lambda cfg, ns: guard
    )
    monkeypatch.setattr(pal_found_filesystem_cli, "AsyncClientFactory", lambda: factory)
...
async def test_main_acl_denied_returns_exit_8_and_skips_sdk(monkeypatch):
    ...
    monkeypatch.setattr(pal_found_filesystem_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_filesystem_cli,
        "AccessControlGuard",
        lambda cfg, ns: MagicMock(
            check=MagicMock(
                side_effect=pal_found_filesystem_cli.AccessControlError("blocked")
            )
        ),
    )
    ...
    assert rc == pal_found_filesystem_cli.EXIT_ACCESS_CONTROL
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (FileNotFoundError("missing"), pal_found_filesystem_cli.EXIT_NOT_FOUND),
        (
            pal_found_filesystem_cli.AccessControlError("blocked"),
            pal_found_filesystem_cli.EXIT_ACCESS_CONTROL,
        ),
