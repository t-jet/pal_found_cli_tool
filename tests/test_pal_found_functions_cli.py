...
def _ns(**kwargs):
    ...
    for spec in pal_found_functions_cli.OP_SPECS:
...
def test_operation_catalog_has_7_unique_operations():
    ...
    assert len(paths) == 7
    assert pal_found_functions_cli.PAGINATED_OPS == frozenset()


@pytest.mark.parametrize("spec", pal_found_functions_cli.OP_SPECS)
def test_parser_accepts_every_canonical_operation(spec):
    parser = pal_found_functions_cli.build_parser()
...
async def test_invoke_dispatches_every_canonical_operation(spec):
    ...
    await pal_found_functions_cli._invoke(
        spec["resource"],
        spec["operation"],
        client,
        args,
...
async def test_invoke_parses_json_and_boolean_args():
    ...
    await pal_found_functions_cli._invoke("query", "execute", client, args, timeout=7)
...
def test_get_client_routes_root_and_nested_clients():
    ...
    assert pal_found_functions_cli._get_client(cfg, "query", factory) == sdk.functions.Query
    assert (
        pal_found_functions_cli._get_client(cfg, "version_id", factory)
        == sdk.functions.ValueType.VersionId
    )


def test_model_to_dict_converts_bytes_to_length_envelope():
    assert pal_found_functions_cli._model_to_dict(b"abc") == {"bytes": 3}
...
async def test_main_success_uses_acl_retry_output_and_b3_scope(monkeypatch, capsys):
    ...
    monkeypatch.setattr(pal_found_functions_cli, "ConfigLoader", _Cfg)
    monkeypatch.setattr(pal_found_functions_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_functions_cli, "AccessControlGuard", lambda cfg, ns: guard
    )
    monkeypatch.setattr(pal_found_functions_cli, "AsyncClientFactory", lambda: factory)
...
async def test_main_acl_denied_returns_exit_8_and_skips_sdk(monkeypatch):
    ...
    monkeypatch.setattr(pal_found_functions_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_functions_cli,
        "AccessControlGuard",
        lambda cfg, ns: MagicMock(
            check=MagicMock(
                side_effect=pal_found_functions_cli.AccessControlError("blocked")
            )
        ),
    )
    ...
    assert rc == pal_found_functions_cli.EXIT_ACCESS_CONTROL
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (FileNotFoundError("missing"), pal_found_functions_cli.EXIT_NOT_FOUND),
        (
            pal_found_functions_cli.AccessControlError("blocked"),
            pal_found_functions_cli.EXIT_ACCESS_CONTROL,
        ),
