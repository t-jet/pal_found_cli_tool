...
pal_found_ontologies_cli = importlib.import_module("pal_found_ontologies_cli")
...
def _ns(**kwargs):
    ...
    for spec in pal_found_ontologies_cli.OP_SPECS:
...
def test_operation_catalog_has_67_unique_operations():
    ...
    assert len(pal_found_ontologies_cli.OP_SPECS) == 67
    assert len(paths) == 67


@pytest.mark.parametrize("spec", pal_found_ontologies_cli.OP_SPECS)
def test_parser_accepts_every_canonical_operation(spec):
    parser = pal_found_ontologies_cli.build_parser()
...
async def test_invoke_dispatches_every_canonical_operation(spec, tmp_path):
    ...
    if spec["binary_download"]:
        ...
        monkeypatch.setattr(
            pal_found_ontologies_cli, "BinaryDownloadHandler", lambda config=None: handler
        )
        try:
            result = await pal_found_ontologies_cli._invoke(
                spec["resource"],
                spec["operation"],
                client,
                args,
        ...
        await pal_found_ontologies_cli._invoke(
            spec["resource"],
            spec["operation"],
            client,
            args,
...
def test_get_client_routes_nested_and_root_clients(monkeypatch):
    ...
    assert (
        pal_found_ontologies_cli._get_client(cfg, "action", factory)
        == sdk.ontologies.Action
    )
    assert (
        pal_found_ontologies_cli._get_client(cfg, "action_type", factory)
        == sdk.ontologies.Ontology.ActionType
    )
    assert (
        pal_found_ontologies_cli._get_client(cfg, "query_type", factory)
...
def test_paginated_catalog_uses_page_size_and_token_specs():
    ...
    assert pal_found_ontologies_cli.PAGINATED_OPS == expected
...
async def test_invoke_paginated_batches_pages():
    ...
    helper = pal_found_ontologies_cli.PaginationHelper(page_size=1, batch_pages=2)

    result = await pal_found_ontologies_cli._invoke_paginated(
...
async def test_binary_upload_reads_body_file_and_sets_attachment_headers(tmp_path):
    ...
    args = _args_for(
        pal_found_ontologies_cli.OPERATION_BY_RESOURCE["attachment"]["upload"],
...
async def test_media_upload_does_not_send_attachment_headers(tmp_path):
    ...
    body_file.write_bytes(b"abc")
    spec = pal_found_ontologies_cli.OPERATION_BY_RESOURCE["media_reference_property"][
        "upload"
    ]
...
async def test_main_success_uses_acl_retry_output_and_b3_scope(monkeypatch, capsys):
    ...
    monkeypatch.setattr(pal_found_ontologies_cli, "ConfigLoader", Cfg)
    monkeypatch.setattr(pal_found_ontologies_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_ontologies_cli, "AccessControlGuard", lambda cfg, ns: guard
    )
    monkeypatch.setattr(pal_found_ontologies_cli, "AsyncClientFactory", Factory)
    monkeypatch.setattr(pal_found_ontologies_cli, "RetryHandler", lambda: retry)
    monkeypatch.setattr(
        sys, "argv", ["prog", "ontology", "get", "ontology-rid", "--format", "json"]
    )

    rc = await pal_found_ontologies_cli.main()

    assert rc == pal_found_ontologies_cli.EXIT_SUCCESS
...
async def test_main_acl_denied_returns_exit_8(monkeypatch):
    ...
    monkeypatch.setattr(pal_found_ontologies_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_ontologies_cli,
        "AccessControlGuard",
        lambda cfg, ns: MagicMock(
            check=MagicMock(
                side_effect=pal_found_ontologies_cli.AccessControlError("blocked")
            )
        ),
    )
    ...
    assert rc == pal_found_ontologies_cli.EXIT_ACCESS_CONTROL
...
async def test_main_user_input_error_returns_exit_1(monkeypatch):
    ...
    monkeypatch.setattr(pal_found_ontologies_cli, "LogSetup", MagicMock())
    monkeypatch.setattr(
        pal_found_ontologies_cli, "AccessControlGuard", lambda cfg, ns: MagicMock()
    )
    monkeypatch.setattr(pal_found_ontologies_cli, "AsyncClientFactory", Factory)
    monkeypatch.setattr(pal_found_ontologies_cli, "RetryHandler", lambda: retry)
    ...
    assert rc == pal_found_ontologies_cli.EXIT_USER_INPUT
...
@pytest.mark.parametrize(
    "exc,exit_code",
    [
        (PermissionError("403"), pal_found_ontologies_cli.EXIT_PERMISSION_DENIED),
        (FileNotFoundError("missing"), pal_found_ontologies_cli.EXIT_NOT_FOUND),
        (TimeoutError("slow"), pal_found_ontologies_cli.EXIT_TIMEOUT),
        (RuntimeError("boom"), pal_found_ontologies_cli.EXIT_SERVER_ERROR),
    ],
)
...
def test_output_formatter_supports_json_and_toon():
    json_out = pal_found_ontologies_cli.OutputFormatter(format_setting="json").format(
        {"key": "value"}
    )
    toon_out = pal_found_ontologies_cli.OutputFormatter(format_setting="auto").format(
        [
            {"a": 1, "b": 2},
            {"a": 3, "b": 4},
        ]
...
def test_resolve_converts_kebab_to_snake():
    assert (
        pal_found_ontologies_cli._resolve("object-type", "get-full-metadata")
        == "get_full_metadata"
    )
