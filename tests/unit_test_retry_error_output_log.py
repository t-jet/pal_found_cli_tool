...
class TestRetryHandlerExecute:
    ...
    @pytest.mark.asyncio
    async def test_execute_wraps_each_attempt_in_wait_for(
...
class TestErrorSerializerExitCodeMapping:
    ...
    def test_validation_error_exit_code_1(self, capsys):
        ...
        ser = ErrorSerializer()
        code = ser.serialize(
            _SDKValidationError("invalid input"), print_to_stdout=False
        )
    ...
    def test_rate_limit_error_exit_code_7(self, capsys):
        ...
        ser = ErrorSerializer()
        code = ser.serialize(
            _SDKRateLimitError("too many requests"), print_to_stdout=False
        )
    ...
    def test_module_not_found_exit_code_9(self, capsys):
        ...
        ser = ErrorSerializer()
        code = ser.serialize(
            ModuleNotFoundError("missing module"), print_to_stdout=False
        )
        assert code == EXIT_CONFIGURATION
...
class TestLogSetupConfigure:
    ...
    def test_configure_all_supported_levels(self, clean_log_env):
        ...
        for level_name in SUPPORTED_LEVELS:
            ...
            root = logging.getLogger()
            assert root.level == getattr(logging, level_name), (
                f"Failed for {level_name}"
            )
